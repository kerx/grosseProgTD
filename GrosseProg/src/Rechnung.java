...
public class Rechnung {
	...
	/**
	 * Methode um verarbeite zu verkleinern und den Code übersichtlicher zu gestalten
	 * @param input
	 */
	public void einlesen(ArrayList<String> input){
		...
		for(String str : input){
			...
			if(name.equals("") && str.startsWith("%")){
				...
				if(split.length == 3){
					...
					for(int i = 0; i<3;i++){
						...
						try{
							...
							throw new IllegalArgumentException("Nicht genau 3 Zahlen als Strategie angegeben");
						}
					}
				...
				throw new IllegalArgumentException("Mehrere Kommentar- oder Strategiezeilen am Stück");
			}
		}
	}
		...
		for(int i = 0;i<3;i++){
			if(strategie[i] <= 0){
				...
				while(sum < abschnittslaengen[i]){
					...
					sum += strategie[i];
					if(sum <= 240){
						// Nur wenn der Endtermin (Praxis geschlossen) nicht überschritten wird
	 ...
	 * @return
	 */
	private boolean berechneZeit(int count, int wz, int mwz, int mwz_sum, int lz, ArrayList<Integer> terminlaenge, Terminplan termin){
		if(terminlaenge.isEmpty()){
			...
			for(int dauer: termindauern){
				int terminlaenge_save = terminlaenge.get(terminlaenge.size()-1);
				...
				if(berechneZeit(count+1, wz_new, mwz_new, mwz_sum_new+mwz_new, lz_new, (ArrayList<Integer>) terminlaenge.clone(), termin)){
					return true;
				}else{
